...
class DBHandler {
    ...
    static void createFullDB() {
        ...
        try {
            ...
            stmt.executeUpdate("CREATE TABLE inventory(" + "ID varchar(20) NOT NULL," + "Desc varchar(255)," + "COGS varchar(12), " + "Date_Made DATE, " + "Sale_Date DATE, " + "Sale_Price varchar(7))");
            c.close();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    static void addRec(String id, String desc, String cogs, String dateMade) throws SQLException {
        Connection c = connect();
        PreparedStatement stmt = c.prepareStatement("INSERT INTO inventory (ID, Desc, COGS, Date_Made, Sale_Date) " + "VALUES (?,?,?,?, null)");
        stmt.setString(1, id);
    ...
    static void update(String[] items) {
        Connection c = connect();
        try {
            PreparedStatement stmt = c.prepareStatement("UPDATE inventory SET " + "Desc = ?, COGS = ?, Date_Made = ?, Sale_Date = ?, Sale_Price = ? WHERE ID = ?");
